...
- (void)animateMenuViewWithShow:(BOOL)show
{
    ...
    if (show) {
        ...
        self.backGroundView.frame = CGRectMake(0, 0, self.frame.size.width, UIScreen.mainScreen.bounds.size.height);
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self.backGroundView.backgroundColor = [UIColor colorWithWhite:0.f alpha:0.f];//0.3f
            if (downType == ZHFilterMenuDownTypeTwoLists) {
            ...
            for (ZHFilterModel *filterModel in modelArr) {
                if (filterModel.selectFirst) {
                    if (filterModel.multiple) {
                        ...
                        if ([((ZHFilterModel*)[filterModel.itemArr firstObject]) selected]) {
                            continue;
                        }
                    }
                    if (selected) {
